...
class GovernanceClass(object):
    ...
    def vote(self, trivechaind, signal, outcome):
        return self.go.vote(trivechaind, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, trivechaind):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(trivechaind, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
