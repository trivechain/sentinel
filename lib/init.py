...
def has_trivechain_conf():
    import config
    import io

    valid_trivechain_conf = False

    # ensure trivechain_conf exists & readable
    #
    # if not, print a message stating that Trivechain Core must be installed and
    # configured, including JSONRPC access in trivechain.conf
    try:
        f = io.open(config.trivechain_conf)
        valid_trivechain_conf = True
    except IOError as e:
        print(e)

    return valid_trivechain_conf
...
def main():
    ...
    if not has_trivechain_conf():
        print("TrivechainCore must be installed and configured, including JSONRPC access in trivechain.conf")
        sys.exit(1)
