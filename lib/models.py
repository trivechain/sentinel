...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, trivechaind):
        golist = trivechaind.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_trivechaind(trivechaind, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_trivechaind(self, trivechaind, rec):
        import decimal
        ...
        # exclude any invalid model data from trivechaind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from trivechaind, with every run
        ...
        # get/create, then sync payment amounts, etc. from trivechaind - Trivechaind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(trivechaind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from trivechaind! %s" % e)
            govobj.vote_delete(trivechaind)
            return (govobj, None)
    ...
    def vote_delete(self, trivechaind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(trivechaind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, trivechaind, signal, outcome):
        import trivechainlib
        ...
        output = trivechaind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = trivechainlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(trivechaind, signal)

    def sync_network_vote(self, trivechaind, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = trivechaind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 trivechain addr, non-multisig
            if not trivechainlib.is_valid_trivechain_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Trivechain address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Trivechain Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (trivechainlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not trivechainlib.is_valid_trivechain_address(addr, config.network):
    ...
    def hash(self):
        import trivechainlib
        return trivechainlib.hashit(self.serialise())
