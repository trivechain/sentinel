...
def is_valid_trivechain_address(address, network='mainnet'):
    ...
    trivechain_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Trivechain address
        return False

    if (address_version != trivechain_version):
        return False
...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.trivechaind_serialise())
...
def SHIM_serialise_for_trivechaind(sentinel_hex):
    from models import GOVOBJ_TYPE_STRINGS

    # unpack
    obj = deserialise(sentinel_hex)

    # shim for trivechaind
    govtype_string = GOVOBJ_TYPE_STRINGS[obj['type']]

    # superblock => "trigger" in trivechaind
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # trivechaind expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    trivechaind_hex = serialise(obj)
    return trivechaind_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('trivechain.conf')
